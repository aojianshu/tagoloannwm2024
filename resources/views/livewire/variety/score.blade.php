<div x-cloak x-data="{ openModal: false }">
    ...
    <x-primary-button @click="openModal = !openModal" class="w-full justify-between">
        ...
        @if(Auth::user()->admin != 1)
          @if(!$contestant->variety->where('judge_id', Auth::user()->judge->id)->isEmpty())
          <div>
            <span class="bg-gray-700 text-gray-200 rounded-full py-1 px-2">
              {{ $contestant->variety->where('judge_id', Auth::user()->judge->id)->first()->score }}
            </span>
          </div>
          @endif
        @endif
              ...
              <div class="bg-gray-50 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                  ...
                  <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                    <h3 class="text-lg font-medium leading-6 text-gray-700" id="modal-title">Score Contestant {{ $contestant->name }} | Variety Show</h3>
